...
#define L_IT 0
#define L_EN 1
    int d=0,v=0,q=0,r=1,lang=L_EN,c,i,j,block=0,flush=0;
    char path[512]="";
    char buf[256];
    ...
    while ((c = getopt_long(argc, argv, "bfvqihn:t:d:p:r:",long_options,&option_index)) != -1)
        switch (c)
        {
        case 'h':    //guida
            if(!lang) printf("hid_test [opzioni] [dati]\n"
                ...
                "-f, flush\tadd openprog flush to end of packet\n"
                "-h, help\tguida\n"
    ...
    if (flush) {
        buf[j++] = FLUSH;
        if (v) printf("flush (%#04x) appended\n", FLUSH);
    }
    for (;j<n;j++) buf[j]=0;
